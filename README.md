...
2. 次のいずれかでルートを指定します。
   - 登録済み GPX ボタンを押す、または手元の `.gpx` ファイルを選択する。
   - 「地図から断面図」で住所・地名を検索して地図を移動し、「2点選択を開始」してから地図上の2点をクリックする（2点間の直線上の標高をサンプリングして断面図を作ります）。
3. 標高が無い点がある場合は、標高 API により補完されます。
4. 地図、断面図、傾斜図を確認します。
5. 必要に応じて「補完済み GPX を保存」から `.gpx` を保存します。
6. 「印刷・エクスポート」で A4 / A3 などの用紙比率と強調比を調整し、出力形式（PNG / PDF）を選んで断面図または傾斜図を保存します。
